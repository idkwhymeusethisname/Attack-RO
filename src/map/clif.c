...
void clif_parse_ViewPlayerEquip(int fd, struct map_session_data* sd)
{
	...
	else if( (pc_readaccountreg(tsd,add_str("#PRIVATE_VIEWEQUIP")) > 0 && tsd->status.show_equip) || (pc_has_permission(sd, PC_PERM_VIEW_EQUIPMENT) && (pc_readaccountreg(tsd,add_str("#PRIVATE_VIEWEQUIP")) < 1 || pc_get_group_level(sd) >= 80)) )
