...
void initChangeTables(void)
{
	...
	StatusIconChangeTable[SC_EMERGENCYCALL] = SI_EMERGENCYCALL;
	...
	StatusChangeFlagTable[SC_PNEUMA] |= SCB_NONE;
	...
	StatusDisplayType[SC_PNEUMA] = BL_PC;
...
void status_calc_misc(struct block_list *bl, struct status_data *status, int level)
{
	...
	if (bl->type&battle_config.enable_perfect_flee) {
		stat = status->flee2;
		stat += status->luk + 100; // (every 10 luk = +1 perfect flee)
...
int status_get_sc_def(struct block_list *src, struct block_list *bl, enum sc_type type, int rate, int tick, unsigned char flag)
{
	...
	switch (type) {
		...
		case SC_STASIS:
			...
			tick_def2 = (status->vit + status->dex) * 1;//แก้ให้ติดสถานะ
			break;
		case SC_WHITEIMPRISON:
			...
			if( bl->type == BL_PC )
				...
				tick_def2 = (status->vit + status->luk)*500;//แก้ให้ติดสถานะ
			break;
		case SC_BURNING:
		...
		case SC_VACUUM_EXTREME:
			tick_def2 = (sd ? sd->status.str : status_get_base_status(bl)->str) * 10;
			break;
		case SC_KYOUGAKU:
...
//		if (status_get_race2(bl) == RC2_GVG && status_sc2scb_flag(type)&SCB_MAXHP) return 0;
	...
	switch(type)
	{
		...
		case SC_BITESCAR: {
				...
				val2 = (status_get_max_hp(bl) * (val1 + (b_status->dex / 60))) / status_get_max_hp(bl); // MHP% damage
				tick_time = 1000;
				val4 = tick / tick_time;
			}
			break;
