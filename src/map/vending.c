...
void vending_purchasereq(struct map_session_data* sd, int aid, int uid, const uint8* data, int count)
{
		...
		if(battle_config.extended_vending){
			if(vsd->vend_loot == ITEMID_ZENY || !vsd->vend_loot) {
				...
				for(k = 0; k < MAX_INVENTORY; k++)
					if(sd->cart.u.items_inventory[k].nameid == vsd->vend_loot)
						loot_count += sd->cart.u.items_inventory[k].amount;
	...
	**/
	if(battle_config.extended_vending){
		if(vsd->vend_loot == ITEMID_ZENY || !vsd->vend_loot) {	
			...
			for( i = 0; i < MAX_INVENTORY; i++)
				if(sd->cart.u.items_inventory[i].nameid == vsd->vend_loot)
				{
					struct item *item;
					item = &sd->cart.u.items_inventory[i];
	...
	for( i = 0; i < count; i++ ) {
		...
		if(battle_config.ex_vending_info){ // Extended Vending system [Lilith]
		char temp[256];
		sprintf(temp, msg_txt(sd,1598), sd->status.name, (int)z, vsd->vend_loot?itemdb_jname(vsd->vend_loot):"Zeny");
		clif_disp_message(&vsd->bl, temp, strlen(temp), SELF);
	}
...
int8 vending_openvending(struct map_session_data* sd, const char* message, const uint8* data, int count, struct s_autotrader *at)
{
	...
	if( Sql_Query( mmysql_handle, "INSERT INTO `%s`(`id`, `account_id`, `char_id`, `sex`, `map`, `x`, `y`, `title`, `autotrade`, `body_direction`, `head_direction`, `sit`, `extended_vending_item`) "
		...
		vendings_table, sd->vender_id, sd->status.account_id, sd->status.char_id, sd->status.sex == SEX_FEMALE ? 'F' : 'M', map[sd->bl.m].name, sd->bl.x, sd->bl.y, message_sql, sd->state.autotrade, at ? at->dir : sd->ud.dir, at ? at->head_dir : sd->head_dir, at ? at->sit : pc_issit(sd), sd->vend_loot ) != SQL_SUCCESS ) {
		Sql_ShowDebug(mmysql_handle);
	}
